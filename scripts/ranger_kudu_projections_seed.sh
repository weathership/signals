#!/usr/bin/env bash
# Seed ranger.* Kudu projection tables via Impala HS2 and register PG foreign tables.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
HS2_SQL="${HS2_SQL:-/tmp/hs2_sql}"
DDL="$ROOT/config/ranger/kudu_projections.sql"
FDW_SQL="$ROOT/config/ranger/kudu_projections_fdw.sql"
...
echo "OK: ranger Kudu projections + foreign tables"
