#!/usr/bin/env bash
# M3 CI gate: publish CE → wait for signals_eventing_ci (or signals_ci) success.
set -euo pipefail
...
DAG_ID="${AIRFLOW_EVENTING_CI_DAG:-${AIRFLOW_EVENTING_SMOKE_DAG:-signals_eventing_ci}}"
CE_TYPE="${SIGNALS_EVENTS_CI_TYPE:-${SIGNALS_EVENTS_SMOKE_TYPE:-dev.signals.eventing.ci}}"
TIMEOUT="${AIRFLOW_EVENTING_CI_TIMEOUT:-${AIRFLOW_EVENTING_SMOKE_TIMEOUT:-180}}"

info() { echo "eventing-ci: $*"; }
die() { echo "ERROR: eventing-ci: $*" >&2; exit 1; }
...
for i in $(seq 1 30); do
  ...
  sleep 2
  if [[ "$i" -eq 30 ]]; then
    info "DAG $DAG_ID not listed — falling back to signals_ci"
    DAG_ID=signals_ci
  fi
done
...
bash "$ROOT/scripts/signals_events_publish.sh" "$CE_TYPE" \
  "{\"ci\":true,\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\"}" \
