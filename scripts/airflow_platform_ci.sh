#!/usr/bin/env bash
# Trigger signals_ci DAG once and wait for success (lab M2 CI gate).
...
DAG_ID="${AIRFLOW_CI_DAG_ID:-${AIRFLOW_SMOKE_DAG_ID:-signals_ci}}"
TIMEOUT="${AIRFLOW_CI_TIMEOUT:-${AIRFLOW_SMOKE_TIMEOUT:-180}}"

info() { echo "airflow-ci: $*"; }
die() { echo "ERROR: airflow-ci: $*" >&2; exit 1; }
...
run_id="manual__ci__$(date -u +%Y%m%dT%H%M%SZ)"
...
print(json.dumps({
  ...
  "conf": {"source": "signals_airflow_platform_ci"},
}))
PY
)"

info "triggering dag_run_id=$run_id dag_id=$DAG_ID"
