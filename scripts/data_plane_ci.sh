#!/usr/bin/env bash
# Post-up CI gate for Kudu + Impala (lab). Invoked from stack-ready when enabled.
# Elevated system gate — use `just data-plane-ci`. One-off: scripts/data_plane_smoke.sh.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

info() { echo "data-plane-ci: $*"; }
die() { echo "ERROR: data-plane-ci: $*" >&2; exit 1; }
...
if [[ "$(uname -s)" != "Linux" ]]; then
  info "Darwin: skip Impala CI probe"
  exit 0
fi
