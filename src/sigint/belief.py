...
class FrameOfDiscernment:
    ...
    This keeps the focal set at ~220 elements for 175 leaves.
    """
    ...
    def _build_focal_elements(self) -> None:
        """Pre-compute the restricted focal element set."""
        ...
        for cat in cs.all_categories:
            if cat.code in cs.leaf_codes:
                continue  # skip leaves (already singletons)
            desc = cs.descendants(cat.code)
            if desc and desc != cs.leaf_codes:  # skip if same as theta
    ...
    def all_focal_elements(self) -> list[FocalElement]:
        """All focal elements in the restricted set."""
    ...
    def singleton(self, code: str) -> FocalElement:
        """Get the singleton focal element for a leaf code."""
        return self._singletons[code]

    def internal(self, code: str) -> FocalElement:
        """Get the internal node focal element for a parent code."""
    ...
    def conflict(self) -> float:
        """Not applicable at frame level; returns 0."""
        return 0.0
